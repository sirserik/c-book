...
Разница между ними ровно одна: в `struct` поля и методы по умолчанию публичные, в `class` — приватные. Больше никаких отличий нет; всё остальное — соглашения, о которых договорились программисты.
...
Конвенция:
Соглашение такое: `struct` берут для простых наборов данных без поведения — `Point`, `Color`, результат деления с частным и остатком. Поля там торчат наружу, и это нормально: прятать нечего. `class` берут там, где есть инкапсуляция, инварианты и нетривиальная логика, — то есть почти везде.
...
У игрока есть имя, текущее здоровье и максимальное здоровье. И три действия: получить урон, вылечиться, ответить на вопрос, жив ли он.
...
Первое `const` относится к возвращаемому типу: наружу отдаётся ссылка на неизменяемую строку, и через неё поле не испортить. Второе, в конце сигнатуры, делает метод константным — это обещание не менять состояние объекта, и компилятор его проверяет: попытка написать внутри `name_ = ...` не соберётся.
...
Есть случаи, когда список инициализации не роскошь, а единственный способ. Это поля с типом `const` — присвоить им нельзя, только инициализировать. Поля-ссылки — с ними ровно та же история. И поля тех классов, у которых нет конструктора без параметров: создать их «пустыми», а потом присвоить, не выйдет.
...
`Location` хранит:
У локации есть короткий идентификатор для ссылок из кода и файла мира, отображаемое название, описание и словарь выходов — отображение из направления в идентификатор соседней локации.
