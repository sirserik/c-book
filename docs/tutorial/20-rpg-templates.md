...
Расплата за это трёхчастная. Каждый файл, использующий шаблон, компилирует его тело заново — отсюда долгая сборка проектов, где шаблонов много. Реализация оказывается на виду у всех, спрятать её в `.cpp` не получится. И один и тот же шаблон порождается в нескольких единицах трансляции; дубликаты потом убирает компоновщик, но время на их создание уже потрачено.
...
```cpp
// C++11: раскрываем пакет рекурсией — печатаем первый аргумент,
// остальные передаём дальше, пока не кончатся.
void print_all() {
    std::cout << "\n";                       // база рекурсии: пакет пуст
}

template <typename First, typename... Rest>
void print_all(const First& first, const Rest&... rest) {
    std::cout << first << " ";
    print_all(rest...);                      // на один аргумент меньше
}
```

В C++17 то же самое пишется одной строкой через свёртку: `((std::cout << args << " "), ...)`. Но нам нужна версия для C++11, и рекурсия по пакету — стандартный для неё приём.

Обратите внимание, что база рекурсии — **не шаблон**, а обычная функция без аргументов. Когда пакет пустеет, вызов `print_all()` находит именно её, и рекурсия заканчивается.
...
Итого недостатки шаблонов: долгая компиляция, потому что каждый файл порождает все используемые шаблоны заново; раздутый объектный код до этапа компоновки; и сообщения об ошибках, в которых развёрнут каждый уровень подстановки. Последнее исправили только в C++20, добавив концепты, — об этом в главе 51.
