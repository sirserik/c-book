...
- Имя.
- Вес.
- Возможность отобразить в инвентаре.

У каждого свои особенности:
- Weapon: урон.
- Armor: защита.
- Consumable: что делает при использовании.
...
- **Никогда не присваивайте объект потомка в переменную базы.** Только указатели или ссылки.
- Если нужно «как-то хранить разнотипные объекты» — `std::vector<Item*>` или `std::vector<std::unique_ptr<Item>>` (глава 18).
...
- Если объект **реально** `Weapon` или его потомок — возвращает `Weapon*`.
- Иначе — `nullptr`.
...
**База `Item`**:
- Виртуальный деструктор `virtual ~Item() = default;` — обязательно.
- Запрет копирования/перемещения через `= delete` — защита от slicing.
